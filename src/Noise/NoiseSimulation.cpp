...
//Sara begin       
        ...
        switch (index) {
            ...
            case 10: *vector = m_calculation.SPL_tipvortexdB()[opPointIndex]; zeroY = true; break;
            ...
            case 15: *vector = m_calculation.SPLadB3d()[opPointIndex]; zeroY = true; break;
            case 16: *vector = m_calculation.SPLsdB3d()[opPointIndex]; zeroY = true; break;
            case 17: *vector = m_calculation.SPLpdB3d()[opPointIndex]; zeroY = true; break;
            case 18: *vector = m_calculation.SPL_LEdB3d()[opPointIndex]; zeroY = true; break;
            case 19: *vector = m_calculation.SPL_LEdBAW3d()[opPointIndex]; zeroY = true; break;
            case 20: *vector = m_calculation.SPL_LEdBBW3d()[opPointIndex]; zeroY = true; break;
            case 21: *vector = m_calculation.SPL_LEdBCW3d()[opPointIndex]; zeroY = true; break;
            case 22: *vector = m_calculation.SPL_LBLVSdB3d()[opPointIndex]; zeroY = true; break;
            case 23: *vector = m_calculation.SPL_bluntdB3d()[opPointIndex]; zeroY = true; break;
            case 24: *vector = m_calculation.SPL_tipvortexdB3d()[opPointIndex]; zeroY = true; break;
            case 25: *vector = m_calculation.SPLdB3d()[opPointIndex]; zeroY = true; break;
            case 26: *vector = m_calculation.SPLdBAW3d()[opPointIndex]; zeroY = true; break;
            case 27: *vector = m_calculation.SPLdBBW3d()[opPointIndex]; zeroY = true; break;
            case 28: *vector = m_calculation.SPLdBCW3d()[opPointIndex]; zeroY = true; break;
        ...
        switch (index) {
            ...
            case 39: *vector = m_calculation.SPLdB3d_final_rotor_loops()[opPointIndex]; zeroY = true; break;
            case 40: *vector = m_calculation.SPLdBAW3d_final_rotor_loops()[opPointIndex]; zeroY = true; break;
            case 41: *vector = m_calculation.SPLdBBW3d_final_rotor_loops()[opPointIndex]; zeroY = true; break;
            case 42: *vector = m_calculation.SPLdBCW3d_final_rotor_loops()[opPointIndex]; zeroY = true; break;
            case 43: *vector = m_calculation.SPLadB3d()[opPointIndex];zeroY = true; break;
            case 44: *vector = m_calculation.SPLsdB3d()[opPointIndex];zeroY = true; break;
            case 45: *vector = m_calculation.SPLpdB3d()[opPointIndex];zeroY = true; break;
            case 46: *vector = m_calculation.SPL_LEdB3d()[opPointIndex]; zeroY = true; break;
            case 47: *vector = m_calculation.SPL_LEdBAW3d()[opPointIndex];zeroY = true; break;
            case 48: *vector = m_calculation.SPL_LEdBBW3d()[opPointIndex]; zeroY = true; break;
            case 49: *vector = m_calculation.SPL_LEdBCW3d()[opPointIndex]; zeroY = true; break;
            case 50: *vector = m_calculation.SPL_LBLVSdB3d()[opPointIndex]; zeroY = true; break;
            case 51: *vector = m_calculation.SPL_bluntdB3d()[opPointIndex]; zeroY = true; break;
            case 52: *vector = m_calculation.SPL_tipvortexdB3d()[opPointIndex]; zeroY = true; break;
            case 53: *vector = m_calculation.SPLdB3d()[opPointIndex]; zeroY = true; break;
...
void NoiseSimulation::exportCalculation(QTextStream &stream) {
    ...
    stream << Qt::endl;
    if(m_parameter.Lowson_type==0){stream << "leading edge model: Von Karman" << Qt::endl;}
    if(m_parameter.Lowson_type==1){stream << "leading edge model: Rapid Distortion" << Qt::endl;}//era m_calculation.m_
    ...
    for (int i = 0; i < noiseOpPoints.size(); ++i) {
        ...
        stream << Qt::endl;
               if((m_parameter.LE_check) & (m_parameter.LBLVS!=0) & (m_parameter.blunt_check!=0) & (m_parameter.tipvortex_check!=0)){
        stream << qSetFieldWidth(14) <<
               ...
               else if((m_parameter.LE_check) & (m_parameter.LBLVS!=0) & (m_parameter.blunt_check!=0) & (m_parameter.tipvortex_check==0)){
                   stream << qSetFieldWidth(14) <<
                             "Freq [Hz]" <<
                             "SPLa" <<
                             "SPLs" <<
               ...
               else if((m_parameter.LE_check) & (m_parameter.LBLVS==0) & (m_parameter.blunt_check==0)){
                                  stream << qSetFieldWidth(14) <<
                                            "Freq [Hz]" <<
                                            "SPLa" <<
                                            "SPLs" <<
...
void NoiseSimulation::exportCalculationqs3DNoise_blade(QTextStream &stream) {
    ...
    stream << Qt::endl;
    if(m_parameter.Lowson_type==0){stream << "leading edge model: Von Karman" << Qt::endl;}
    if(m_parameter.Lowson_type==1){stream << "leading edge model: Rapid Distortion" << Qt::endl;}
    ...
    if (m_parameter.LE_check){stream << "SPL LE: " << m_calculation.Final_qs3d_LE;}
...
stream << "Section: " << (i+1)<<"/"<<number_of_segments << Qt::endl;
        stream << Qt::endl;
if((m_parameter.LE_check) & (m_parameter.LBLVS!=0) & (m_parameter.blunt_check!=0)){
        stream << qSetFieldWidth(14) <<
                  "Freq [Hz]" << ";" <<
                  "SPLa" << ";" <<
                  "SPLs" << ";" <<
...
void NoiseSimulation::exportCalculationqs3DNoise_rotor(QTextStream &stream) {
    ...
    int number_of_segments = pBEM->m_pBData->m_pos.size();
     ...
     if (m_parameter.LE_check){stream << "SPL LE: " << m_calculation.Final_qs3d_LE_rotor_loops;}
...
QVariant NoiseSimulation::accessParameter(Parameter::NoiseSimulation::Key key, QVariant value) {
    ...
    case P::qs3d_check:
        if(set) m_parameter.qs3d_check = value.toBool();
        else value = m_parameter.qs3d_check; break;
...
break;
    ...
    case P::vegetation_check:
        if(set) m_parameter.vegetation_check = value.toBool();
        else {value=m_parameter.vegetation_check;}
break;

    case P::atm_check:
        if(set) m_parameter.atm_check = value.toBool();
        else {value=m_parameter.atm_check;}
break;

    case P::rel_humidity:
        if(set) m_parameter.rel_humidity = value.toInt();
        else {value=m_parameter.rel_humidity;}
break;
    ...
    case P::LE_check:
        if(set) {m_parameter.LE_check = value.toBool();}
else {value=m_parameter.LE_check;}
        break;
